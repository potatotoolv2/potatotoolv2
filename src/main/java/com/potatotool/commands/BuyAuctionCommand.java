...
public class BuyAuctionCommand {
   ...
   private static int execute(CommandContext<FabricClientCommandSource> context, AuctionNotificationManager notifier) {
      ...
      if (client == null || client.player == null) {
         ...
         boolean ok = notifier.requestAutoBuy(rawUuid, client);
         if (!ok) {
            ((FabricClientCommandSource)context.getSource()).sendError(Component.literal("§cInvalid auction uuid."));
            return 0;
         } else {
            ((FabricClientCommandSource)context.getSource()).sendFeedback(Component.literal("§aAuto-buy requested for §e" + rawUuid));
            return 1;
         }
      }
   }
