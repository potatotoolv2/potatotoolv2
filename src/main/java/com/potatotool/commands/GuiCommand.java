...
public class GuiCommand {
   public static void register(CommandDispatcher<FabricClientCommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)ClientCommands.literal("scannergui").executes(context -> openSettings(context)));
      dispatcher.register((LiteralArgumentBuilder)ClientCommands.literal("scannersettings").executes(context -> openSettings(context)));
   }
   ...
   private static int openSettings(CommandContext<FabricClientCommandSource> context) {
      ...
      if (client == null) {
         ...
         ((FabricClientCommandSource)context.getSource()).sendFeedback(Component.literal("§aScanner settings opening..."));
         return 1;
      } else {
